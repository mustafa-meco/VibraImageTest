...
amplitude_vibraimage = np.zeros_like(frames, dtype=np.float32)
...
# The source's Eq. 2 is more involved. This is a basic illustration.
# We'll use a simple temporal difference for frequency approximation.
# frequency_vibraimage = np.abs(frames[-1].astype(np.float32) - frames.astype(np.float32))
# Convert frames list to a NumPy array
frames_array = np.array(frames, dtype=np.float32)

# Frequency Vibraimage (Simplified approach)
frequency_vibraimage = np.abs(frames_array[-1] - frames_array)
